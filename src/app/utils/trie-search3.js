...
class Trie {
    constructor(){
        ...
        this.findAllOccurencesOfSearchTermInTrie = function findAllOccurencesOfSearchTermInTrie (searchterm, node) {
        ...
        // 1 : FIND ALL OCCURENCES OF FIRST LETTER
            ...
            let findFirstLetter = function findFirstLetter(firstLetter, node) {
                ...
                for (const [key, value] of node.keys ) {
                    ...
                    if ( currentLetter !== firstLetter ) {
                        console.log(currentLetter,' :THIS LETTER IS NOT AT ROOT, BUT LOOKING IN ITS KEYS ::::::::::::::');

                        for (const [key, value] of currentLetterOwnMap.keys ) {
                            let currentInnerLetter = key, currentInnerLetterOwnMap = value;
                            
                            if ( currentInnerLetter !== firstLetter ) {
                                console.log(currentLetter,' :THIS LETTER IS NOT IN THE CURRNT MAP, BUT LOOKING INTO NEXT ONE ------>');
                                continue; // go for the next key of map
                                
                            } else if ( currentInnerLetter === firstLetter ) {
                                console.log('WE HAVE A MATCH !!!!!!! ::::::::::::::THIS KEY IS A MATCH,', currentInnerLetter, '===', firstLetter);
                                matchingNodesForLetter.push(node); // store node where matching 1ST letter
                                return; // go for the next key of map : continue seacrhing for other occurences
                            }
                        }

                    } else if ( currentLetter === firstLetter ) {  // FIRST LETTER IS FOUND AT ROOT
                        console.log('WE HAVE A MATCH !!!!!!! ::::::::::::::THIS KEY IS A MATCH,', currentLetter, '===', firstLetter);
                        matchingNodesForLetter.push(node); // store node where matching 1ST letter
                        return; // go for the next key of map : continue seacrhing for other occurences
                    }
                }
            ...
            findFirstLetter(firstLetter, node);
        };
